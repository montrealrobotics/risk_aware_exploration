...
if __name__ == "__main__":
    ...
    # env setup
    envs =  make_env(args, args.seed + 0, 0, args.capture_video, run_name, args.gamma) 
    ...
    os.makedirs(os.path.join(storage_path, "0"))
    os.makedirs(os.path.join(storage_path, "1"))
        ...
        for step in range(0, args.num_steps):
            ...
            if not done:
                ...
                try:
                    ...
                    pass
                #ep_cost += np.array(cost); cum_cost += np.array([info["final_info"][0]["cost"]])
            if cost == 1:
                subdir = '1'
            else:
                subdir = '0'
            im = next_obs['vision']*255
            im = Image.fromarray(np.array(im).astype(np.uint8))
            im.save(os.path.join(storage_path, subdir, "%d_%d.png"%(episode, step)))
            if done:
                envs.reset()
                break
    envs.close()
